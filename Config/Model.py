...
class DataHelper(object):
	...
	def getList(self):
		list=[['','','请您填写手机/邮箱/用户名'],['admin','',u'请您填写密码'],['admin','admin',u'请输入验证码']]
		return list
	...
	def readExcel(self,rowValue,colValue):
		book=xlrd.open_workbook(Config.data_dirs()+'/login.xlsx')
		sheet=book.sheet_by_index(0)
		return sheet.cell_value(rowValue,colValue)

	def readExcels(self,filename):
		rows=[]
		book=xlrd.open_workbook(Config.data_dirs()+filename)
		sheet=book.sheet_by_index(0)
